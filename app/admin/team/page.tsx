...
export default function TeamAdmin() {
    ...
    const [form, setForm] = useState({ name: '', role: '', image: '', facebook: '', twitter: '', linkedin: '', order: 0 });
    ...
    function openModal(item?: TeamMember) {
        if (item) {
            setEditing(item);
            setForm({ name: item.name, role: item.role, image: item.image, facebook: item.facebook || '', twitter: item.twitter || '', linkedin: item.linkedin || '', order: item.order });
        } else {
            setEditing(null);
            setForm({ name: '', role: '', image: '', facebook: '', twitter: '', linkedin: '', order: members.length });
        }
        setShowModal(true);
    }
        ...
        <div>
            <div className={styles.card}>
                <div className={styles.cardHeader}>
                    <h2 className={styles.cardTitle}>فريق العمل ({members.length})</h2>
                    <button className={`${styles.btn} ${styles.btnPrimary}`} onClick={() => openModal()}>+ إضافة عضو</button>
                </div>

                {members.length === 0 ? (
                    <div className={styles.emptyState}><p>لا يوجد أعضاء. أضف أول عضو!</p></div>
                ) : (
                    <table className={styles.table}>
                        <thead><tr><th>الصورة</th><th>الاسم</th><th>المنصب</th><th>الترتيب</th><th>الإجراءات</th></tr></thead>
                        <tbody>
                            {members.map((item) => (
                                <tr key={item.id}>
                ...
                <div className={styles.modal}>
                    <div className={styles.modalContent}>
                        <div className={styles.modalHeader}>
                            <h3 className={styles.modalTitle}>{editing ? 'تعديل العضو' : 'إضافة عضو جديد'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className={styles.form}>
                            ...
                            <div className={styles.formRow}>
                                <div className={styles.formGroup}>
                                    ...
                                    <input className={styles.formInput} value={form.facebook} onChange={(e) => setForm({ ...form, facebook: e.target.value })} />
                                </div>
                                <div className={styles.formGroup}>
                                    <label className={styles.formLabel}>تويتر</label>
                                    <input className={styles.formInput} value={form.twitter} onChange={(e) => setForm({ ...form, twitter: e.target.value })} />
                                </div>
                            </div>
                            <div className={styles.formRow}>
                                <div className={styles.formGroup}>
                                    <label className={styles.formLabel}>لينكدإن</label>
                                    <input className={styles.formInput} value={form.linkedin} onChange={(e) => setForm({ ...form, linkedin: e.target.value })} />
                                </div>
                                <div className={styles.formGroup}>
